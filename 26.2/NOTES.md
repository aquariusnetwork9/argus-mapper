...
Not ported yet: waystone teleports (`Waystones`, `WaystoneMarkers`, the GUI's Waystones tab, the World Map waypoint menu item and `/argus waystone`; the API client and parsing is in core), the bounty markers (`Bounty`, `BountyMarkers`, the GUI's Bounty tab and `/argus bounty`; the parsing and fetching is in core) and auto-map (`AutoMapper`, the calibration flight, the GUI's Auto-map tab, the map right-click option and `/argus automap`); it is in the 1.21.x builds only for now.
