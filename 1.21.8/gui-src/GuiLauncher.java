...
/**
 * 1.21.8's GuiLauncher - identical to fabric-common's copy (same 4-arg KeyBinding constructor,
 * no API differences to work around here), except for the static block below. Forked rather than
 * left as fabric-common's shared copy purely so the pause-menu button registration is 1.21.8-only
 * without touching a file 1.21.4 also compiles - keeps each version's own javap-verified
 * additions isolated to that version's own files, matching the pattern 1.21.4 and 1.21.11 both
 * already use.
 */
